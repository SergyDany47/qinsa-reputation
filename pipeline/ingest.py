...
def run_incremental_ingest(restaurant_id, *, max_reviews=10, generate_replies=True,
                           model="gemini-2.5-flash", trigger="manual", emit=_noop) -> dict:
    """
    ...
    `trigger` ("manual" | "scheduled") solo etiqueta el registro en `ingest_runs`;
    cada ejecución (éxito o error) queda registrada para trazabilidad.
    """
    restaurant = get_restaurant_by_id(restaurant_id)
    if not restaurant:
        # Sin fila de restaurante no hay FK válida → no se registra run.
        raise ValueError("Restaurante no encontrado.")
    if not restaurant.get("google_maps_url"):
        log_ingest_run(restaurant_id, trigger, "error",
                       error="El restaurante no tiene google_maps_url.")
    ...
    try:
        ...
        if not reviews:
            ...
            log_ingest_run(restaurant_id, trigger, "success", scraped=0, inserted=0)
        ...
        log_ingest_run(restaurant_id, trigger, "success", scraped=len(reviews), inserted=inserted)
        return {"inserted": inserted, "scraped": len(reviews)}

    except Exception as e:
        # Registra el fallo para trazabilidad y propaga (el SSE/scheduler ya lo manejan)
        log_ingest_run(restaurant_id, trigger, "error", error=str(e))
        raise
